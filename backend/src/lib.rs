...
async fn capabilities(State(state): State<AppState>) -> Json<serde_json::Value> {
    let endpoints: Vec<_> = state
        ...
        .collect();
    Json(
        serde_json::json!({"beta":true,"mail":!endpoints.is_empty(),"endpoints":endpoints,"calendar":false,"backups":false,"sent_copy":false}),
    )
}
async fn security_headers(request: Request, next: Next) -> Response {
    ...
    headers.insert("content-security-policy",HeaderValue::from_static("default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self'; connect-src 'self'; frame-ancestors 'none'; form-action 'self'; base-uri 'none'"));
    headers.insert(
        "permissions-policy",
