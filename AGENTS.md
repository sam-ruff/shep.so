...
Defer performance measurements while the PC is saturated with other work; run them at the end on an otherwise idle machine. `python3 scripts/e2e.py --functional-only` runs correctness flows without the latency benchmark and accepts unittest's `-k` patterns to select scenarios. Never weaken thresholds based on a loaded-host result.
...
**Every AI-driven MCP scenario must have an equivalent automated scenario in `scripts/e2e.py` (or a saved fixture consumed by it).** Both AI and automated runs use the same MCP server and real native controls. Do not replace mouse clicks with direct calls to update functions. The state file is an observation-only oracle, never an action API. Save screenshots for visual review; JSON state alone cannot prove layout quality. Saved scenarios click fixed sidebar and Move dialog rows: a unified entry or fixture folder added above the account trees moves every row beneath it (39 px per sidebar row, 32 px for the centred dialog), so update the saved coordinates together with the pinned order test in `src/ui/folder_tests.rs`.
...
The `test_rapid_*` family in `scripts/e2e.py` drives long, fast move and Undo sequences (alternating keyboard and mouse archive/trash, Move dialog moves of the same and neighbouring rows, bulk selections, drag and drop, cross-account moves, the same under `mail_actions="slow"` and `background_sync=True`, and a `restart` after each) and checks the list against the store after every step. Two test-support observations back it: `store_truth` is the store's own answer to the open query (`Command::StoreTruth` through the ordinary read lane and `Store::truth`, `src/store/truth.rs`), carrying the page ids, totals, unread and per-account Inbox counts, every folder's totals and the pending move/bulk/ledger totals, with `fresh` (no backend event or dispatched command since it was asked) and `agrees` (fresh, and the projected page, totals and Inbox counts equal it); `drawn_rows` records the mail rows the widget tree actually drew in the latest frame with their window positions (`src/ui/draw_log.rs`, opened by the root `ContextArea` and appended by each row) and reports duplicates, stale ids, out-of-order slices and overlaps as `consistent`. Wait for `mail_pending == 0`, then `store_truth.agrees` and `drawn_rows.consistent`; the Python `assert_store_matches` helper also compares the reader, sidebar Inbox count and private-bus badge. Neither observation is an action API, and neither proves presented pixels; keep the WebP review.

The `test_live_*` family runs the same flows against a real server. `desktop.start(live_imap=True)` is test-support only: it launches without `--demo`, so the production engine, 5 second checks, IDLE watcher, folder discovery and real MOVE/flag commands all run, with a fresh data root under the owned `XDG_DATA_HOME`, a memory-only keychain (`tests/support/live.rs`; the OS keychain is never read) and one IMAP account built from `SHEP_LIVE_IMAP_HOST`, `SHEP_LIVE_IMAP_PORT`, `SHEP_LIVE_IMAP_USER`, `SHEP_LIVE_IMAP_PASSWORD`, `SHEP_LIVE_SMTP_HOST` and `SHEP_LIVE_SMTP_PORT`. Only `--test-state` observation is added. The scenarios skip when the variables are unset, so the default suite stays offline. The mailbox is the disposable `e2e@shep.so` on the shep.so Stalwart server (its password is the infrastructure vault key `vault_mail_password_e2e`, read in-process and exported, never printed or written to the repository); `scripts/live_mailbox.py` wipes every folder, removes folders earlier runs created (never the special-use ones), APPENDs the same 120 Inbox and 4 `Deleted Items` messages, and delivers one more for the IDLE check, while the test's own IMAP connection verifies folder totals with STATUS/SEARCH only. Never point the live mode at `sam@shep.so` or any personal mailbox, and leave the mailbox seeded or wiped, never with stray folders.
...
`engine/account_work.rs` owns account and calendar scheduling in a background coordinator. Its request channel and waiting queue each hold at most 32 entries; one-shot channels grant work, interrupt read-only sync and acknowledge completion. A mail action (`write`: move, archive, delete, flag, undo, bulk, send, move recovery) is granted immediately while that account's sync is downloading; it uses its own connection and never interrupts or waits for the download. Writes keep FIFO order among themselves per account, a second sync is refused while one runs, and a sync may start beside an active write. Only `exclusive` lifecycle work (account save/test, removal, restore, profile import, folder creation and folder jobs, calendar writes) interrupts a read-only sync and waits for its cache commits to settle; a queued sync yields to a waiting exclusive request without starting a download. Separate accounts proceed independently, and abandoned requests must release their place. Do not restore shared account/calendar mutex maps, and do not make mail actions exclusive again.

Because a download and an action overlap, `store/write_ledger.rs` reconciles them: `patch_flags` and cache relocation record acknowledged writes (id and kind only, in the session scratch database, at most 256 per account), `download_account` takes a ledger epoch before its first SELECT, and `apply_sync_since`, `sync_message_since` and `sync_staged_message_since` rank a write acknowledged after that epoch above the check's listings. A stale `Flags` item keeps the local flags, a stale `Reconcile` keeps a moved-in row, and a stale body for a moved-away id is dropped. Finishing a check clears the account's entries acknowledged before it began; entries older than the 600 s cycle bound are pruned when a check begins. The rules are in `docs/agents/mail.md`; keep the decisions pure and unit-tested. CONDSTORE (`providers/mail/condstore.rs`, desktop feature `condstore`) saves a folder's SELECT HIGHESTMODSEQ/UIDVALIDITY in `folder_modseqs` only after that folder's flags and listing were sent, and never while a ledger write in the folder outranks the check; restore and account removal forget an account's values. Anything unexpected (no saved value, new UIDVALIDITY, NOMODSEQ, lower value, rejected CHANGEDSINCE) uses the full flag listing. With a saved value and QRESYNC, `providers/mail/qresync.rs` enables it before SELECT and replaces `UID SEARCH ALL` with `CHANGEDSINCE n VANISHED`; the feature-gated `Vanished` item removes cached rows with the Reconcile protections (restored pins, pending-move rows, later moved-in rows). Only a result whose cached minus vanished plus new equals SELECT's EXISTS, with no live expunge/arrival during the reply, is used; anything else takes the complete UID listing. The `FolderState` and `Vanished` sync items are feature-gated because the Flutter and backend matches are exhaustive.

`engine/mail_sync.rs` schedules every account independently: each ready account gets its own check task through the shared provider semaphore, and a slow account's running check never delays another account's next check. `engine/account_sync.rs` keeps an owned task until already-started cache writes settle, even when the containing account check is cancelled. Interrupt only read-only provider/setup work, and only for exclusive lifecycle work, engine shutdown and the cycle timeout. Drain received cache items before releasing account ownership; `try_join!` would detach an active SQLite `spawn_blocking` operation on provider error and can let old state overwrite a newer write. An interrupted check is not a complete folder listing or Inbox baseline. Preserve the deterministic held-provider/cache-commit, failure, timeout, cancelled-owner, bounded-queue and ordering regressions.

The saved native held-sync flows use `desktop.start(held_account_sync=true)`. They exercise the production coordinator/download pipeline against an indefinitely held fictional provider, without real network or keychain access. Closing must persist an explicitly selected message's read-on-leave change; flagging must complete while the held provider keeps its download (`account_sync_waiting` stays true), and a failed write must restore the flag and allow another attempt without touching the download. Actual personal-server sync/close diagnosis and temporary close-to-tray remain separate unfinished work.
...
`src/ui/` owns presentation and small caches. `engine.rs` bridges bounded channels and background work. `store/worker.rs` owns the mail-cache SQLite connection and local operation leases on one thread, behind a bounded 32-command FIFO. Accepted SQL jobs drain even if their observation is cancelled or the last Store handle is dropped; a request cancelled before admission must not become a later write. Local lease release uses a one-shot channel's closure, so it cannot wait for queue space. Keep worker ordering/cancellation/failure tests, temporary selection tables and independent-process file leases intact. Long database copies must use a separate online snapshot path rather than holding this worker for the entire transfer. `engine/lifecycle_work.rs` owns the Google connection, connection lifecycle and calendar setup lanes: each is a bounded 32-request FIFO coordinator granting shared or exclusive access through one-shot channels, so an abandoned request leaves the queue, a holder releases by drop (including on error) and close drains active holders before the coordinator exits. `providers/google/owner.rs` owns the token vault on one thread behind a bounded FIFO; an admitted refresh or sign-in finishes and persists even when its caller is cancelled. Do not restore the engine RwLock/Mutex fields or the shared token mutex. `providers::MailProvider`, `CalendarProvider`, and `backup::BackupProvider` are extension points: add a provider without teaching the UI its wire protocol.
...
Native ordinary-cache receiving enables shared `staged-receive`: partial IMAP FETCH or streamed POP3 after smaller mail, owned immutable staging, bounded MIME previews and atomic incremental SQLite publication. Check encrypted-cache policy before plaintext staging. Download progress resets the inactivity deadline; shutdown drains admitted cache writes. Preserve requested `MailDetail.body_limit` through paging, refresh and late results, independently of visible text length. Encrypted staging, fully streamed reading and client receiving parity remain open.

Current practical limits and unsupported behavior must remain explicit in README: encrypted large-message staging and full streaming remain unfinished, the raw-mail snapshot ceiling is 256 MiB, HTML/text rendering is static with policy-controlled external images, Google Gmail uses app passwords rather than Gmail OAuth, the calendar sync window is -90/+365 days, and recurring CalDAV edits belong in the server calendar UI. Improve these deliberately; do not hide them with success messages.
...
Body downloads use two lanes (`providers/mail/lanes.rs`): inline bodies go smallest first, newest first among equal sizes, in 10-message / 4 MiB batches; bodies over `SLOW_LANE_BYTES` (1 MiB) are fetched one at a time after every folder's inline bodies and before the >25 MiB staged pass. `Reconcile` still carries the complete listing because the store only deletes cached rows absent from it, and a deferred body is not cached. `InboxSyncFinished` follows the last Inbox body in whichever lane carries it. A slow-lane failure leaves the already delivered messages committed; the rest stay unknown and are retried next check. `imap_sync_orders_small_bodies_first_and_slow_bodies_after_every_folder` scripts both the success and the failure path.
...
`engine/google_lifecycle.rs` takes exclusive Google lane access before exclusive connection lifecycle access. Google provider work (status checks, calendar sync, event edits, Drive backups, restore, profile sync sessions) holds shared Google lane access; login, disconnect and cleanup hold exclusive access, and the lane is strict FIFO so provider work queued behind a disconnect waits for it. Do not invert that order with calendar/account/removal/restore lanes, and do not cancel keychain mutations at the generic provider timeout. Show the cached workspace before startup cleanup. If deletion fails, clear in-memory tokens, preserve the pending cleanup state and offer a retry; reconnect must finish that cleanup first. The credential store's FIFO covers delete as well as write so an older blocking save cannot restore a deleted login.
...
Preferences shows one **Sign in with Google** button and never OAuth client fields (R75). `providers/google/client.rs` owns Shep's installed-app client: the ID and secret come from `option_env!("SHEP_GOOGLE_CLIENT_ID"/"SHEP_GOOGLE_CLIENT_SECRET")`, a runtime override exists only in debug and `test-support` builds (an empty ID simulates no client), and a build without both disables the button with "Google sign-in is not configured in this build." New sign-ins and staged-candidate resumption use only this build's client. `google_client_id`/`google_client_secret` are legacy values kept only so a grant from a self-configured client keeps refreshing; `refresh_secret` prefers this build's secret for its own grants, then that stored secret, then the vault's. Sign-in uses the system browser, a random `127.0.0.1` loopback port, PKCE S256, an exact 256-bit state, a three-minute bound and a UI cancellation token (Cancel sign-in, and quitting). Cancelling stops only the browser wait; a received code always completes through the token owner. Denied, refused, cancelled, timed-out and expired-code outcomes are typed `SignInError`s that leave the connection unchanged. Keep the fake-browser loopback tests and see [Google sign-in client](docs/agents/google-sign-in.md) for build configuration and Google Cloud setup.
...
The OS credential entry is a bounded two-grant vault, accepting the original single-grant format on read. Stage the candidate alongside the grant selected by `Preferences.google_grant.id`. Service validation runs against that candidate only. `Store::activate_google` atomically commits the non-secret grant ID/client/access, verified Drive identity, lifecycle revision and refreshed/archived calendar access. It rejects changed legacy client values and cleanup state. The DB pointer is the activation point: a crash before it keeps the previous login; a crash afterward selects the candidate even before credential pruning. Never put token material in SQLite.
...
The active OAuth client is the committed grant's issuing client, distinct from this build's sign-in client. Backups keep using its client/verified Drive destination until a new sign-in commits, and a self-configured grant keeps refreshing with its own client until then. Switching clients changes the Drive target, whose app data belongs to the other Google Cloud project. Backend-owned grant metadata survives late preference saves/results. A Calendar-only grant pauses Drive automation but retains its prior destination/history; a Drive-only grant archives cached Google calendars. Source setup, refresh and backup restore cannot bypass the current read-only scope.
...
Preferences search indexes actual editable sections in `ui/settings_search.rs`; update that index when adding settings. Search results open the matching controls. Preserve icon-only tooltip behavior and both tooltip preferences. Unread Inbox counts cover the cache independent of the current query. The Move chooser shows the server's INBOX as Inbox and marks its first result with the Enter hint. With `foreign_move_folders` (effective only with `cross_account_moves`), rows from other IMAP accounts carry `badge_inline` with the sidebar's account name and open `Dialog::MoveConfirm` instead of moving; the preference checkbox sits at the end of the Reading and layout card so saved General-tab coordinates above it hold.

Settings help uses `App::with_help` and the topics in `ui/help_tip.rs`: a small raised 11 px **?** mark in a 16 px focusable target (24 px pointer reach) that shows its tip on hover, click or Tab focus. Its own profile-synced `help_icons` preference (Show help icons beside settings, independent of icon tooltips) omits it entirely when off, so no empty Tab stop remains. Add help only for easily misunderstood settings, in short plain UK English. The icon must not grow its row; Tab in Preferences reveals the focused control through `ui/focus_reveal.rs`. The test-support `help_tips` observation records drawn icons (content coordinates) and tip bounds (window coordinates); keep the `test_settings_help_*` native scenarios.
...
Move search Enter resolves the latest form text in `Message::MoveFirst`; do not capture the first destination in the prior frame's text-input submit message. Keep the native fast typing/Enter regression and the no-redraw state test. A first result from another account becomes `Message::MoveForeign`, which stores `App.move_confirm` and opens `Dialog::MoveConfirm`; Enter/Y sends `ConfirmMove`, which rechecks the message id, the destination account (present and IMAP) and both preferences before `transfer_mail`, and Escape/N sends `CancelMoveConfirm`, which returns to `Dialog::Move` with the typed query and refocuses the search field. `Close` and `open` clear the pending choice. In selection mode a foreign row starts a group review with `Intent::Move { foreign: true }`, which skips the one-message auto-confirm, so `Dialog::BulkReview` is the confirmation. Explicit pick-list accounts keep moving immediately and never produce foreign rows.
...
`engine/mail_sync.rs` starts a check after cached Ready and uses a saved seconds-based interval, default 5 and configurable from 5–3600. Database version 5 moves a saved interval still on the previous 15 second default to 5 once; any other saved value, and a 15 chosen after that migration, is kept. `sync_minutes` remains the separate calendar cadence for compatibility. Do not restore a minutes-long mail default. Settings changes update a watch channel only after persistence; unrelated saves do not reset the schedule. Each account is scheduled on its own: one loop owns the timer, lists the ready accounts on every pass and spawns a separate check for each account that is idle and due, so a slow account never delays another. An account whose check is still running is skipped until it finishes. A manual refresh runs each account once more after its current check, coalescing repeated clicks into one follow-up per account. Manual clicks update the refresh control immediately and the manual busy key stays set until every account has served the request; automatic checks use a separate busy key that stays set while any account is checking. Each account flushes its cached changes, requests pending move recovery and republishes the workspace as soon as it finishes.

The sync worker shares the provider semaphore and cannot consume foreground-read or persistence workers. The 600-second timeout or a failure releases only that account and permits retry. Typed `MailSyncFinished` results carry the account key, and a recovered account clears only its own earlier sync error; successful explicit settings saves clear only their settings error. Preserve unrelated errors. Keep optimistic mail overlays and open context menus through background refreshes.

`engine/mail_push.rs` keeps one IMAP IDLE watcher per listed IMAP account on its own connection (`providers/mail/push.rs`), outside the account lock and the provider semaphore. The watcher SELECTs INBOX and idles; any untagged EXISTS, EXPUNGE or FETCH marks that account due through `Schedule::wake`, so its normal check starts at once (a running check gets one follow-up). IDLE is re-issued every 25 minutes at the latest, under the 29 minute RFC 2177 limit, and a stop signal sends DONE and LOGOUT. A server without IDLE after login ends the watcher as unsupported with no retry; a lost connection reconnects with delays doubling from 5 seconds to 5 minutes, reset once IDLE connects. Watchers start with the scheduler, stop when the account leaves the listing or the worker shuts down, restart (old one logs out first) when the listed incoming identity (`connection_key` plus the active credential slot) changes, and never replace polling: the interval check still runs for every account and every folder, so a dead watcher or an unsupported server loses nothing. Preview, POP3 and manual-only fixtures never start a watcher.

Virtual-time Rust tests exercise the actual scheduler with object-scoped listing and check implementations: immediate startup, repeated checks, interval changes, queued/coalesced refresh, failure and timeout recovery, a slow account never delaying a fast one, a failing account leaving the other untouched, accounts added or removed between passes, a push starting only that account's check, watcher backoff growth and reset, an unsupported server keeping polling, and removal or shutdown stopping watchers. Scripted-session tests in mail-core cover IDLE after SELECT, the EXISTS signal, DONE and LOGOUT on stop, the 25 minute re-issue and the unsupported result. These are correctness checks, not performance measurements. Native `background_sync: true` uses a delayed fictional arrival; `sync_failure_once: true` fails its first check. Both require test-support and never contact a real mail server. Production startup remains automatic; ordinary native fixtures keep automatic checks off for reproducibility.
...
A completed Kept record retains its token receipt and independent local copy,
but cannot reserve a subsequently rediscovered provider original. Fresh move
preparation releases only that finished source lookup in its own transaction.
An existing original row takes precedence over the old Kept reader alias;
