...
Settings help uses `App::with_help` and the topics in `ui/help_tip.rs`: a 16 px focusable **?** (24 px pointer reach) that shows its tip on hover, click or Tab focus and is omitted entirely when icon tooltips are off, so no empty Tab stop remains. Add help only for easily misunderstood settings, in short plain UK English. The icon must not grow its row; Tab in Preferences reveals the focused control through `ui/focus_reveal.rs`. The test-support `help_tips` observation records drawn icons (content coordinates) and tip bounds (window coordinates); keep the `test_settings_help_*` native scenarios.
