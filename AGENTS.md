...
Multi-selection groundwork lives in `store/selection.rs`; it is not yet connected to native controls or bulk execution. `store/mail_query.rs` owns the common scope/ranking plan for inbox pages and captured membership. Keep selected IDs/ranks in SQLite and return at most one metadata page to iced. `capture_selection` ignores page offset, `change_selection` checks the expected revision atomically, and `freeze_selection` copies exact selected membership for a review. New arrivals cannot silently join an existing selection. Missing mail remains explicit in selected versus available counts. Snapshot metadata pages read current flags/folders; immutable membership does not freeze message content. Callers must release abandoned snapshots, reject stale acknowledgments and clean up through background channels. Temporary selection tables are process-local and disappear on connection close; durable bulk jobs/Undo receipts need their own journal before this becomes the execution path. Preserve `tests/selections.rs` when changing query scopes, ranking, pagination or selection lifecycle.
