...
For long move/Undo sequences use the `test_rapid_*` family: after each step wait for `mail_pending == 0`, then `store_truth.agrees` (the store's own page, totals and Inbox counts equal the displayed list) and `drawn_rows.consistent` (no duplicated, stale, reordered or overlapping drawn rows); both are test-support observations, never action APIs, and the family ends each sequence with a `restart` on a `persistent` fixture.

The `test_live_*` family repeats those flows against a real server through `desktop.start(live_imap=True)`. That launch is test-support only and takes no fixture options: no `--demo`, so the production engine runs (5 second checks, IDLE watcher, real folder discovery and MOVE/flag commands) with a fresh data root under the owned artifact directory, a memory-only keychain and one IMAP account built from `SHEP_LIVE_IMAP_HOST`, `SHEP_LIVE_IMAP_PORT`, `SHEP_LIVE_IMAP_USER`, `SHEP_LIVE_IMAP_PASSWORD`, `SHEP_LIVE_SMTP_HOST` and `SHEP_LIVE_SMTP_PORT`; the harness refuses the launch when any is unset, and the scenarios skip. The only permitted target is the disposable `e2e@shep.so` mailbox on the shep.so Stalwart server, whose password is the infrastructure vault key `vault_mail_password_e2e` (read in-process into the environment, never printed, logged or passed on a command line). Never use `sam@shep.so` or any personal mailbox, the installed app, a personal profile or the OS keychain. Each scenario calls `scripts/live_mailbox.py` first (`Mailbox.reset()`: wipe every folder, delete folders earlier runs created but never the special-use `Deleted Items`, `Junk Mail`, `Sent Items` or `Drafts`, then APPEND the deterministic 120 Inbox and 4 Deleted Items messages), and `Mailbox.deliver()` appends one more for the IDLE check. The test connection otherwise only reads (LIST, STATUS, SEARCH). After every step `assert_live_matches` waits for `mail_pending == 0`, `store_truth.agrees` and `drawn_rows.consistent`, then compares the server's folder totals with the store's and `desktop_badge.count` with the server's unread INBOX count. Leave the mailbox seeded or wiped, never with stray folders. `python3 scripts/live_mailbox.py status|seed|wipe` inspects or resets it by hand.
