...
class Desktop:
    ...
    def start(self, width=1440, height=920, empty_calendars=False, conversation_mail=False):
        self.stop()
        ...
        self.app = subprocess.Popen(
            [str(binary), "--demo", "--test-state", str(self.directory / "state.json"), *(["--empty-calendars"] if empty_calendars else []), *(["--conversation-mail"] if conversation_mail else [])],
...
TOOLS = [
    {"name": "desktop.start", "description": "Launch an isolated Shep fixture workspace on Xvfb. Requires cargo build --profile test-ui --features test-support. No real credentials or network writes.",
     "inputSchema": {"type": "object", "properties": {"empty_calendars": {"type": "boolean", "default": False}, "conversation_mail": {"type": "boolean", "default": False}, "width": {"type": "integer", "default": 1440}, "height": {"type": "integer", "default": 920}}}},
