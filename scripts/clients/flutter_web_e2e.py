...
def main(formatted=False):
    LOGS.mkdir(parents=True,exist_ok=True)
    prefix='flutter-formatted-web' if formatted else 'flutter-web'
    if formatted:
    ...
    with (LOGS/f'{prefix}-build.log').open('w') as log:
        subprocess.run(['flutter','build','web','--target','test/formatted_main.dart' if formatted else 'test/preview_main.dart','--no-web-resources-cdn'],cwd=ROOT/'flutter',stdout=log,stderr=subprocess.STDOUT,check=True)
    ...
    with (LOGS/f'{prefix}-server.log').open('w') as log:
        ...
        try:
            ...
            with (LOGS/f'{prefix}-e2e.log').open('w') as log:
                subprocess.run(['node','flutter/e2e/formatted.mjs'] if formatted else ['npm','--prefix','flutter/e2e','run','web'],cwd=ROOT,env=env,stdout=log,stderr=subprocess.STDOUT,check=True)
        finally:
            process.terminate()
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--formatted',action='store_true');main(parser.parse_args().formatted)
