...
def main(formatted=False, discovery=False, creation=False, enrollment=False):
    LOGS.mkdir(parents=True,exist_ok=True)
    prefix='flutter-enrollment-web' if enrollment else 'flutter-creation-web' if creation else 'flutter-discovery-web' if discovery else 'flutter-formatted-web' if formatted else 'flutter-web'
    if formatted:
    ...
    with (LOGS/f'{prefix}-build.log').open('w') as log:
        subprocess.run(['flutter','build','web','--target','test/profile_enrollment_main.dart' if enrollment else 'test/profile_creation_main.dart' if creation else 'test/profile_discovery_main.dart' if discovery else 'test/formatted_main.dart' if formatted else 'test/preview_main.dart','--no-web-resources-cdn'],cwd=ROOT/'flutter',stdout=log,stderr=subprocess.STDOUT,check=True)
...
if __name__=='__main__':
    ...
    group.add_argument('--formatted',action='store_true'); group.add_argument('--discovery',action='store_true'); group.add_argument('--creation',action='store_true'); group.add_argument('--enrollment',action='store_true')
    args=parser.parse_args(); main(args.formatted,args.discovery,args.creation,args.enrollment)
