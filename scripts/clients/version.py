...
def lock_versions(text, version):
    ...
    for i, block in enumerate(blocks[1:], 1):
        ...
        if name and name[1] in ('shep-mail-core', 'shep-beta-server', 'shep_mobile_native') and '\nsource = ' not in block:
...
def stamp(version, build, root=ROOT):
    ...
    paths = ['flutter/pubspec.yaml', 'web/package.json', 'web/package-lock.json',
             ...
             'shared/mail-core/Cargo.toml', 'flutter/rust/Cargo.toml', 'backend/Cargo.lock', 'Cargo.lock', 'flutter/rust/Cargo.lock']
