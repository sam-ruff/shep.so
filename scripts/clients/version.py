...
def lock_versions(text, version):
    ...
    for i, block in enumerate(blocks[1:], 1):
        ...
        if name and name[1] in ('shep-action-core', 'shep-calendar-core', 'shep-mail-core', 'shep-mail-content', 'shep-profile-core', 'shep-beta-server', 'shep_mobile_native') and '\nsource = ' not in block:
...
def stamp(version, build, root=ROOT):
    ...
    paths = ['flutter/pubspec.yaml', 'web/package.json', 'web/package-lock.json',
             ...
             'shared/action-core/Cargo.toml',
             'shared/calendar-core/Cargo.toml',
    ...
    for p in paths[5:]:
        update = cargo_version if p.endswith('Cargo.toml') else lock_versions
        updated[p] = update(original[p], version)
