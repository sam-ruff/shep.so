...
CREATE TABLE IF NOT EXISTS calendar_actions(id TEXT PRIMARY KEY,status TEXT NOT NULL CHECK(status IN ('queued','running','waiting','succeeded','rejected','uncertain','repair','cancelled')),error TEXT,created INTEGER NOT NULL,mutation TEXT NOT NULL,subject TEXT,admission_request TEXT);
