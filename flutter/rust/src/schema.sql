...
CREATE INDEX IF NOT EXISTS individual_mail_action_runnable ON individual_mail_actions(created,id) WHERE status IN ('queued','waiting');
...
CREATE INDEX IF NOT EXISTS calendar_connection_attempt_status ON calendar_connection_attempts(status,created,id);
CREATE TABLE IF NOT EXISTS removed_calendar_connections(id TEXT PRIMARY KEY,revision INTEGER NOT NULL,credential_slot TEXT,cleanup INTEGER NOT NULL DEFAULT 0);
CREATE TABLE IF NOT EXISTS calendar_credential_cleanup(slot TEXT PRIMARY KEY);
CREATE TABLE IF NOT EXISTS calendar_sources(id TEXT PRIMARY KEY,source TEXT NOT NULL,connection_id TEXT REFERENCES calendar_connections(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS calendar_actions(id TEXT PRIMARY KEY,status TEXT NOT NULL CHECK(status IN ('queued','running','waiting','succeeded','rejected','uncertain','repair','cancelled')),error TEXT,created INTEGER NOT NULL,mutation TEXT NOT NULL,subject TEXT,admission_request TEXT,connection_id TEXT,connection_revision INTEGER,credential_slot TEXT);
CREATE TABLE IF NOT EXISTS calendar_binding(id INTEGER PRIMARY KEY CHECK(id=1),subject TEXT NOT NULL);
