...
class _ProfileSyncControlsState extends State<ProfileSyncControls> {
        ...
        children: [
          ...
          if (sync != null) ...[
            CheckboxListTile(
              ...
              onChanged: d.busy
                  ? null
                  : (value) => d.configureSync(enabled: value ?? sync.enabled),
            ),
            for (final field in profileSettingLabels.keys)
              CheckboxListTile(
                ...
                onChanged: d.busy || !sync.enabled
                    ? null
                    : (value) => d.configureSync(
                        field: field,
                        selected: value ?? sync.fieldEnabled(field),
                      ),
              ),
            Padding(
  ...
  String _status(ProfileDiscovery d, ProfileSyncStatus sync) {
    ...
    if (!sync.enabled) {
      return 'Paused on this device. Other devices are not affected.';
    }
    final parts = <String>[];
    if (d.syncRan) {
      parts.add(
...
class _ProfileSyncReviewScreenState extends State<ProfileSyncReviewScreen> {
  ...
  List<Widget> _versions(
    ...
    return [
      ...
      const SizedBox(height: 12),
      if (!seenAll) const Text('Open every version page before choosing.'),
      OutlinedButton.icon(
