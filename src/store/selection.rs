...
pub(super) fn schema(c: &Connection) -> anyhow::Result<()> {
    c.execute_batch("CREATE TABLE scratch.mail_selections(
        id TEXT PRIMARY KEY, revision INTEGER NOT NULL, frozen INTEGER NOT NULL, query TEXT);
        CREATE TABLE scratch.mail_selection_rows(
...
fn version(c: &Connection, id: MailSelectionId) -> anyhow::Result<(u64, bool)> {
    Ok(c.query_row(
        "SELECT revision,frozen FROM scratch.mail_selections WHERE id=?",
        [id.to_string()],
...
fn position(c: &Connection, selection: MailSelectionId, id: &str) -> anyhow::Result<i64> {
    c.query_row(
        "SELECT position FROM scratch.mail_selection_rows WHERE selection=? AND id=?",
...
fn rebase(c: &Connection, source: MailSelectionId, endpoints: &[&str]) -> anyhow::Result<()> {
    ...
    let query: String = c.query_row(
        "SELECT query FROM scratch.mail_selections WHERE id=?",
        [&key],
        |r| r.get(0),
    )?;
    ...
    c.execute(
        "INSERT INTO scratch.mail_selections(id,revision,frozen) VALUES(?,0,0)",
        [&candidate],
    )?;
    let count = capture_rows(c, &query, &candidate, false, Some(&key))?;
    // Validate against the current query before retaining unavailable choices.
    for endpoint in endpoints {
        let found: bool = c.query_row(
            "SELECT EXISTS(SELECT 1 FROM scratch.mail_selection_rows WHERE selection=? AND id=?)",
    ...
    let mut missing = c.prepare("SELECT id FROM scratch.mail_selection_rows previous
        WHERE selection=? AND selected=1 AND NOT EXISTS(SELECT 1 FROM scratch.mail_selection_rows current WHERE current.selection=? AND current.id=previous.id)
        ORDER BY position")?;
    let mut rows = missing.query(params![key, candidate])?;
    let mut position = count;
    while let Some(row) = rows.next()? {
        let id: String = row.get(0)?;
        c.execute(
            "INSERT INTO scratch.mail_selection_rows VALUES(?,?,?,1)",
            params![candidate, id, position],
        )?;
        position += 1;
    }
    drop(rows);
...
/// Build the ordering index incrementally on encrypted disk, then walk it with
/// one row in Rust. A full-result ORDER BY/window would use SQLite TEMP storage.
fn capture_rows(
    c: &Connection,
    query: &MailQuery,
    selection: &str,
    ...
    let plan = mail_query::Plan::selection(c, query)?;
    let (sql, values) = plan.selection_order();
    c.execute(&sql, rusqlite::params_from_iter(values))?;
    let direction = if query.sort == MailSort::Oldest {
        "ASC"
    } else {
        "DESC"
    };
    let mut ordered = c.prepare(&format!("SELECT id FROM scratch.mail_selection_order ORDER BY priority,missing,score,label COLLATE NOCASE,time {direction},id"))?;
    let mut rows = ordered.query([])?;
    let mut insert = c.prepare("INSERT INTO scratch.mail_selection_rows(selection,id,position,selected)
        VALUES(?1,?2,?3,COALESCE((SELECT selected FROM scratch.mail_selection_rows WHERE selection=?4 AND id=?2),?5))")?;
    let mut count = 0i64;
    while let Some(row) = rows.next()? {
        let id: String = row.get(0)?;
        insert.execute(params![selection, id, count, previous, all])?;
        count += 1;
    }
    drop(rows);
    drop(ordered);
...
fn snapshot(
    ...
    let accounts = c
        .prepare(
            "SELECT m.account,COUNT(*) FROM scratch.mail_selection_rows s
...
impl Store {
    ...
    pub async fn capture_selection(
        ...
        self.run(move |c| {
            ...
            tx.execute("DELETE FROM scratch.mail_selections WHERE id=?", [&key])?;
            ...
            tx.execute(
                "INSERT INTO scratch.mail_selections(id,revision,frozen,query) VALUES(?,?,0,?)",
                params![key, revision as i64, serde_json::to_string(&scope)?],
            )?;
            capture_rows(&tx, &query, &key, all, None)?;
        ...
        self.run(move |c| {
            ...
            match change {
                SelectionChange::Set { id: mail, selected, clear_others } => {
                    let captured: bool = tx.query_row("SELECT EXISTS(SELECT 1 FROM scratch.mail_selection_rows WHERE selection=? AND id=?)",
                    ...
                    position(&tx, id, &mail)?;
                    if clear_others {
                        tx.execute("UPDATE scratch.mail_selection_rows SET selected=0 WHERE selection=?", [&key])?;
                    }
                    tx.execute("UPDATE scratch.mail_selection_rows SET selected=? WHERE selection=? AND id=?",
                ...
                SelectionChange::Range { anchor, target, additive } => {
                    ...
                    tx.execute("UPDATE scratch.mail_selection_rows SET selected=1 WHERE selection=? AND position BETWEEN ? AND ?",
                        params![key,a.min(b),a.max(b)])?;
                }
                SelectionChange::All | SelectionChange::Clear => {
                    tx.execute("UPDATE scratch.mail_selection_rows SET selected=? WHERE selection=?",
                        params![matches!(change,SelectionChange::All),key])?;
                }
            }
            tx.execute("UPDATE scratch.mail_selections SET revision=? WHERE id=?", params![(revision+1) as i64,key])?;
            let result = snapshot(&tx,id,&visible)?;
            tx.commit()?;
            Ok(result)
        }).await
    ...
    pub async fn selected_mail_page(
        ...
        self.run(move |c| {
            ...
            let rows = tx
                ...
                FROM scratch.mail_selection_rows s JOIN selectable_mail m ON m.id=s.id
