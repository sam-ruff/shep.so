...
pub(super) async fn step(
    ...
    match review.phase.as_str() {
        ...
        "fields" | "planning" => {
            ...
            if review.phase == "planning" {
                ...
                return db.run(move |db| sql::finish_fields(db, &key, id)).await;
            }
            ...
            if let Some(field) = fields.first() {
                ...
                next.error = None;
                let key = key.to_owned();
                return db.run(move |db| {
                    let tx = db.transaction()?;
                    let current = sql::read(&tx, &key, id)?;
                    ...
                    sql::write(&tx, &next)?; tx.commit()?; Ok(next)
                }).await;
            }
    ...
    let key = key.to_owned();
    db.run(move |db| {
        let current = sql::read(db, &key, id)?;
        ensure!(
