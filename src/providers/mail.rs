...
async fn tls(host: &str, port: u16) -> anyhow::Result<Tls> {
...
async fn upgrade(host: &str, tcp: TcpStream) -> anyhow::Result<Tls> {
    let connector = tokio_native_tls::TlsConnector::from(native_tls::TlsConnector::new()?);
    connector
        .connect(host, tcp)
        .await
...
async fn imap(
    ...
    let client = if account.incoming_security == ConnectionSecurity::StartTls {
        let tcp = tokio::time::timeout(
            ...
            TcpStream::connect((&*account.host, account.port)),
        )
        .await??;
        ...
        async_imap::Client::new(upgrade(&account.host, client.into_inner()).await?)
    } else {
        let mut client = async_imap::Client::new(tls(&account.host, account.port).await?);
        client
            .read_response()
            .await?
...
async fn pop(account: &Account, password: &SecretString) -> anyhow::Result<PopConnection<Tls>> {
    let mut connection = if account.incoming_security == ConnectionSecurity::StartTls {
        ...
        PopConnection(BufReader::new(
            upgrade(&account.host, plain.0.into_inner()).await?,
        ))
    } else {
        let mut connection = PopConnection(BufReader::new(tls(&account.host, account.port).await?));
        anyhow::ensure!(
