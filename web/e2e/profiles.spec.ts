...
async function capture(page: Page, name: string, region: string) {
  // A preference save redraws Preferences, so the region resolved for the
  // scroll can be replaced mid-action; resolve it again rather than fail.
  for (let attempt = 0; ; attempt++) {
    const area = page.getByRole("region", { name });
    await expect(area).toBeVisible();
    try {
      await area.scrollIntoViewIfNeeded();
      break;
    } catch (error) {
      if (attempt === 4 || !String(error).includes("not attached")) throw error;
    }
  }
