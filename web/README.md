...
Select messages with checkboxes, Ctrl/Shift-click or a remappable Select all shortcut in the focused list. Selection stays across pages; Clear unchecks and Done exits. Captured membership uses SQLite in a private worker with readonly cache snapshots. Review and apply Archive/Delete/Move/read/flag changes to the full selection, then use History for progress, Pause/Resume, Undo and explicit recovery. Group moves stay within each original account; complete native/recovery parity remains open. Dependency provenance is in `public/SQLITE-LICENSE.txt`.
