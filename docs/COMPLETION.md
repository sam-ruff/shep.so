...
| Optional Google login and Drive backups | Browser OAuth/PKCE, app-data scope, encrypted rolling backups; destination-scoped setup/history/results, committed-copy acknowledgment, protected retention and local no-overwrite tests | HTTP contracts for OAuth refresh/Drive, durable ambiguous-upload recovery and genuine Google authorization |
...
4. Exercise Google OAuth refresh, Drive upload/list/restore/retention, POP3 and SMTP through deterministic protocol contracts. Handle pagination loops, partial failures and ambiguous writes explicitly. Backup setup now remembers each successful manual copy's passphrase by destination, acknowledges committed copies independently of cleanup, scopes history/list results and restores, and serializes Google authorization against Drive operations. Local writes do not overwrite existing files and retention protects the new copy. Still implement resumable/reserved Drive uploads with a durable recovery journal, bound Drive pagination/JSON and verify ownership before download/deletion. Validate restored credential IDs against included accounts/calendars before any keychain write and strengthen complete-snapshot validation. The current 600-second provider timeout can still interrupt an upload with an unknown remote outcome; do not treat current setup/retention tests as proof of that recovery path.
