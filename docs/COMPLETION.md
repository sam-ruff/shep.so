...
| Google Calendar and CalDAV | Background sync/create/edit/delete, conditional writes, native all-day editor; CalDAV discovery and multi-calendar chooser; calendar access roles and read-only viewing | Remove/disconnect controls; live server evidence |
...
2. Complete account/calendar lifecycle and make connection errors recoverable without leaving stale sources or credentials. CalDAV discovery/connection testing and Google calendar access roles are now implemented; finish removal/disconnect controls, source-specific credential editing and recovery across app restarts.
