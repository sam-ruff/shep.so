...
new reply, with light and compact dark captures reviewed. Of 42 related native
scenarios, 33 pass and one live scenario skips; the seven that fail
(`composer_preferences_shortcut`, `desktop_badge_archive_delete_move_and_undo`,
`desktop_badge_preference_clears`, `drag_compact_dark_and_large_interface_scale`,
`filtered_preferences_do_not_leave_pixels`, `move_foreign_folder_badge` and
`move_recovery_preferences_entry`) fail identically on an unmodified
`origin/main` build on this host.
...
ships on main with this evidence checkpoint. It adds durable desktop Send
preparation, folder creation and account/calendar removal; browser checked
folder creation/rename/move/delete; and Flutter Google calendar admission,
receipt repair, authenticated subject binding and exact lost-reply recovery.
The authenticated Calendar gateway shares Google transport and grant renewal
with the existing provider owners. Network renewal no longer holds the global
grant lock; current sessions and connection generations are checked before
credentials are returned. AGENTS.md records these invariants.

### Verification

- Native review of ten selected messages in a 100,000-message mailbox passes
  20 real pixel cycles: **32.31 ms p95**, maximum 32.73 ms. Confirmation feedback
  passes at **22.14 ms p95**, maximum 22.74 ms, with provider capacity held.
  Both unchanged targets are 100 ms. The final report is
  `artifacts/performance/actions.json`; reviewed capture `3f6da406de91` shows
  updated rows/counts and visible Undo. Metadata stays at most 50 rows and seven
  cached bodies; handler p95 stays below 0.2 ms.
- The separate large-mailbox correctness flow retains full database comparison
  and passes after Undo (`010a375f4c3c`). Native folder/Preparing controls pass
  14 scenarios; removal passes ten distinct controls across a combined run and
  one readiness-wait rerun. All 47 bulk/calendar/connection integration tests
  and the three original pending-read/delete/Undo regressions pass.
- Flutter passes 153 Rust tests after shared HTTP extraction, 234 full Flutter
  tests and 29 final focused controls/FFI tests. These cover exact request
  retention, changed/repeated input, disposal, bounded unresolved requests,
  coherent editor-close recovery, source identity and cache-only repair.
  The compact recovery golden is reviewed; analysis and Clippy are clean.
- Browser folder work passes 235 focused unit tests, 22 control/storage scenarios
  and three separate 100,000-message export/index/selection scenarios with
  concurrent draft saves. TypeScript and production build pass. Shared calendar
  model/owned HTTP tests pass eight cases. Backend passes 67 tests, with one
  configured browser prerequisite ignored, and Clippy is clean.
- Normal implementation hooks pass 1,586 Rust test executions with ten ignored.
  The owned calendar HTTP cases are exercised explicitly in the shared gate.
  Release/client packaging passes 46 tests. Native harness/pixel/gate tests and
  strict documentation builds pass. The common performance gate validates raw
  paired cycles, binary fingerprint, fixture conditions and retention bounds;
  local and Linux quality scripts now run the action measurement.

The first pixel run retained a failing 264.86 ms review p95. Investigation
showed the optional test-only StoreTruth scan contending on the same database
worker: 4,903,500 SQLite VM steps, versus 2,600 for the ten-row review. Timing
now excludes that optional observer; ordinary correctness still requires it.
The failed report remains in the isolated lane as
`native-action-latency-quiet.json`, capture `20392cfc8eef`. No production query
semantics or timing budget was relaxed. Earlier calendar verification exposed
editor metadata loss, stale replies, receipt ordering and incomplete lost-reply
handling; their corrected regressions are included above. Failing evidence is
retained rather than treated as a clean initial run.

### Assumptions and remaining scope

- Pixel timing measures the Linux light-layout fixture, with agent builds/tests
  stopped and ordinary user applications untouched; it is not monitor scanout.
- Selection establishment is setup. These results measure review readiness and
  confirmation, not initial capture of all 100,000 rows.
- Mail is synthetic and provider capacity is held. Owned protocol fixtures do
  not establish live Google, IMAP, SMTP or CalDAV behaviour.
- Apple, Windows, mobile suspension and broader cross-browser execution remain
  separate requirements; the personal installed app was not replaced.
- This is a verified migration checkpoint, not complete app-wide adoption.
  Flutter folders/CalDAV, browser Calendar controls and wider activity,
  scheduling and lifecycle coverage remain active in the request list.
