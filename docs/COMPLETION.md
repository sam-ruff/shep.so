...
| Optional Google login and Drive backups | Browser OAuth/PKCE, app-data scope, encrypted rolling backups; destination-scoped setup/history/results, committed-copy acknowledgment, protected retention; durable upload journal and Drive resumable HTTP contracts; validated atomic local restore and missing-password recovery | OAuth refresh/token-rotation contracts, genuine Google authorization and large real-mailbox verification |
...
4. Exercise Google OAuth refresh/token rotation, POP3 and SMTP through deterministic protocol contracts. Drive now has bounded pagination/JSON, ownership checks before media/deletion, pre-generated IDs, 1 MiB resumable chunks, checkpoint-before-transfer, partial acknowledgment handling, checksum verification and restart recovery. The separate upload journal retains exact encrypted bytes across failure and prevents a new reservation from replacing an unresolved copy. Google account identity comes from Drive `about`, tokens are bound to the OAuth client, and reconnecting the same verified account preserves the target. The overall 600-second timeout no longer cuts off progressing backups; per-request/progress bounds remain. Restore now validates credential ownership, calendar sources and message IDs/limits, reconstructs MIME display/search data off-thread, and commits new mail/configuration atomically before missing-password recovery. Existing mail/settings/passwords/drafts are preserved; tests cover late-insert rollback, malformed archives and idempotent encrypted local restore after a locked keychain. Large imports still hold the cache connection during the transaction; finish the independent cached-read connection review before claiming bounded cache latency during restore/export. Add lifecycle controls for abandoned uploads when removing a destination/account, and verify the Drive path with genuine Google authorization; loopback contracts are not live-service evidence.
