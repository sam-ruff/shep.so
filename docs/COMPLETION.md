...
4. Exercise Google OAuth refresh/token rotation, POP3 and SMTP through deterministic protocol contracts. Drive now has bounded pagination/JSON, ownership checks before media/deletion, pre-generated IDs, 1 MiB resumable chunks, checkpoint-before-transfer, partial acknowledgment handling, checksum verification and restart recovery. The separate upload journal retains exact encrypted bytes across failure and prevents a new reservation from replacing an unresolved copy. Google account identity comes from Drive `about`, tokens are bound to the OAuth client, and reconnecting the same verified account preserves the target. The overall 600-second timeout no longer cuts off progressing backups; per-request/progress bounds remain. Still validate restored credential IDs against included accounts/calendars before any keychain write and strengthen complete-snapshot/atomic restore validation. Add lifecycle controls for abandoned uploads when removing a destination/account, and verify the Drive path with genuine Google authorization; loopback contracts are not live-service evidence.
