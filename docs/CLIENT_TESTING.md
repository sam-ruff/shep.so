...
Captured-selection prerequisites have Rust tests in `flutter/rust/src/selection_tests.rs`, bounded-controller tests in `flutter/test/mail_selection_test.dart`, and an actual FFI contract in `native_repository_test.dart`. The 100,000-message fixture verifies cardinality and bounded responses; it is not a latency benchmark. Held selection/provider work and cancelled callers verify independent cached reads/saves and FIFO ownership.

Native selection controls and durable group actions are described in [Mobile group actions](agents/MOBILE_BULK.md). `flutter/rust/src/groups_tests.rs` covers the journal (frozen 125-row staging, immediate projection, newer-intent and already-applied skips, Undo with cancelled and reversed receipts, credential requests, unconfirmed pauses that are never repeated, explicit flag retry, restart classification, abandoned-review retirement, the account-removal fence with a held provider step, and the 20-job/50-item bounds). `flutter/test/mail_groups_test.dart` drives the Dart controller over the synthetic journal in `test/support/group_repository.dart`; `flutter/test/bulk_controls_test.dart` runs `test/support/bulk_controls_scenario.dart` in light and dark through real controls: the Select button, desktop-style checkboxes, a long-press range, captured Select all and Clear, the frozen review with per-account counts, decline, immediate painting, Pause/Resume, Undo, injected failed and unconfirmed steps, History retry and acceptance. The FFI case in `native_repository_test.dart` freezes, executes and undoes a group in the real SQLite journal. From the root:

```sh
python3 scripts/clients/flutter_web_e2e.py --bulk
python3 scripts/clients/android_e2e.py --device emulator-5554 --bulk-only
```

The browser flow (`flutter/e2e/bulk.mjs`, `test/bulk_main.dart` with 125 extra fictional Inbox messages) uses the visible `Select up to here` menu control for the range, because a semantics click is a tap; captures live in `artifacts/flutter/web/bulk/`. The Android runner pairs `bulk_android_test.dart` with `android_bulk_fixture.py`: the shared controls scenario runs in both schemes with the synthetic journal, then a handed-over 130-message POP3 profile executes, undoes and reopens the real native journal; the report must list `bulk-controls-light`, `bulk-controls-dark` and `bulk-native-journal-restart`. The Appium flow `flutter/e2e/bulk_native.mjs` repeats the controls with real touch input and a slowed synthetic step (`SHEP_BULK_STEP_MS`); Android merges each History card header (title, status and expand label) into one clickable node, so the flow matches the expand label inside it. These fixtures do not establish live IMAP execution, cross-account moves, large-group performance or Apple behaviour.
...
The saved `test_google_requested_permissions_*` native scenarios use actual Drive checkbox/Calendar menu controls, saved/reopened settings and fixture-only sign-in refusal. A compact dark flow requests broader next-sign-in permissions, then opens an existing read-only event and verifies access is unchanged. Existing partial-grant/disconnect tests remain. The harness always sets a fictional sign-in client (`google_client="fixture"`) or an empty one (`google_client="none"`) and never passes the caller's; `test_google_sign_in_*` scenarios cover the **Sign in with Google** card, a build without a client and a connection from a self-configured client (`google_legacy_client=True`). No real credentials or Google authorization is used.

`cargo test --all-features google` covers exact requested URL scopes/PKCE, the built-in, override and legacy-grant client choice, a fake browser driving the loopback redirect (exact state, denied, cancelled, timed-out and expired codes), omitted-scope fallback, broader-grant restriction, refresh after editing choices, scope-bound candidate retry before/after restart, changed-settings activation refusal and UI save acknowledgment ordering. Native fixtures establish controls only; the real provider path remains covered by isolated protocol tests, with live Google and mobile/browser consent separate.
