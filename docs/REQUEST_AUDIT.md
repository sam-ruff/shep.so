...
| R108 | Preferences option so the Move dialog matches other accounts' folders at slightly lower priority, badged with the account, with an Enter/Escape confirmation before the cross-account move; all preferences must profile-sync unless sync is off (18 September 2026) | Desktop delivered in `bd15369`: synced `foreign_move_folders` preference, badged foreign rows while typing with a 15 point ranking penalty, Enter/Escape confirmation; client Move flows and the sync-every-preference audit remain open in TODO |
