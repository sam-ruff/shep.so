...
recorded in [Completion](COMPLETION.md). Implementation `f6d9462` is pushed to
main with normal hooks passing; the complete architecture request remains active.
...
Checkpoint `8db87f1` adds native Preparing Outbox, saved folder
...
Normal hooks pass 1,586 Rust executions, ten ignored; the common performance
gate is wired and its evidence passes. Remaining domain migrations and platform
evidence stay active. These checks do not establish complete app-wide adoption.
...
| R72 (client) | Linux app stores/distribution | Flathub packaging prepared in `packaging/flatpak/` with an offline SDK build, validated metainfo and fixture screenshots; build with flatpak-builder, submission and other stores remain in TODO |
...
| R99 | Settings search should search all settings (11 September 2026) | Search-demo catalogue adds control captions, weighted typo matching, cached results and persisted-field coverage review. 23 September: rendered-caption catalogue, individual-control reveal and focus, a guard that fails for an uncatalogued Preferences caption, and the "shared profile" regression fix. Browser/Flutter parity and the unbuilt review states remain open in TODO; see [Preferences search](agents/SETTINGS_SEARCH.md) |
...
| R110 | Faster UI tests using the official `iced_test` crate, alongside the slow native Xvfb suite (26 September 2026) | Branch `test/iced-simulator`: headless harness in `src/ui/simulator_tests/` running the real App and demo engine, thirteen native scenarios ported as fourteen tests plus a same-run snapshot check inside `cargo test --all-features`; native scenarios kept. Timings and limits in [Completion](COMPLETION.md); merge and further ports tracked in TODO |
| R109 | Wider website without in-development copy, WASM live demo with test data, OS-detected Get Shep panel with platform tabs and one-click install commands, no browser beta card, white Shepherd header logo, UX agent review (26 September 2026) | Implemented on main with Playwright, deployment-contract and image checks; staging and production deployment (R94) follow |
| R101 | Mail is slow to arrive and moves fail with "The server did not report its folder namespace"; create missing folders, trash locally when the server refuses, 5 second default check, per-account threading that skips slow items, server push, and actions that do not interrupt sync (14 September 2026) | Recorded as the highest-priority TODO section. Live probe of `sam@shep.so` (Stalwart) confirmed `LIST "<destination>" ""` returns nothing while `LIST "" ""` reports the root, no Archive folder exists, and Trash/Junk/Sent are `Deleted Items`/`Junk Mail`/`Sent Items` by SPECIAL-USE; IDLE, CONDSTORE, QRESYNC and CREATE-SPECIAL-USE are advertised. Implementation lanes started 14 September. Shipped so far: `fe9be95` (small new bodies download before slow ones; bodies over 1 MiB fetched in a second pass) and `9039d70` (namespace discovery via `LIST "" ""`, Archive/Trash/Junk resolved to special-use folders, Archive created with `\Archive`). `0431962` adds the 5 second default and independent per-account checks; `294be90` adds IMAP IDLE push per account; `b70712b` completes refused moves on this device with automatic retry; `5dad5cb` lets actions run beside a live download with a write ledger that stops a completed check undoing them. Every requested item has shipped on desktop; follow-ups and client parity are listed per item in TODO. Branch `feat/imap-push-condstore` (22 September) restarts an IDLE watcher when incoming settings or the credential binding change and adds CONDSTORE `CHANGEDSINCE` flag refresh with a saved per-folder HIGHESTMODSEQ and QRESYNC `VANISHED` expunge discovery, which Sam asked to implement rather than defer (fixture-verified; live confirmation in #12 and Flutter/browser parity remain). The 22 September destination follow-ups (`fix/move-destination-followups`) add the NAMESPACE discovery fallback, a fresh LIST before the first folder sync and acknowledged-folder toasts on desktop |
| R108 | Preferences option so the Move dialog matches other accounts' folders at slightly lower priority, badged with the account, with an Enter/Escape confirmation before the cross-account move; all preferences must profile-sync unless sync is off (18 September 2026) | Desktop delivered in `bd15369`: synced `foreign_move_folders` preference, badged foreign rows while typing with a 15 point ranking penalty, Enter/Escape confirmation. Browser delivered by PR #8 (`feat/web-foreign-move-folders`): shared ranking, both synced preferences, badges, confirmation, group review with listed folders only and gateway cross-account transfer. Flutter Move flow, live IMAP transfer verification and the sync-every-preference audit remain open in TODO |
| CI inbox page | Every PR must have a passing CI build; main's Linux job fails the 100,000-message Inbox page benchmark at p95 703 ms against 50 ms (23 September 2026) | Fixed on `fix/ci-inbox-page-benchmark`: the account-removal filter no longer runs a correlated check per message; budget unchanged, query-plan test and a removed-account benchmark case added. Confirmed on the runner: Inbox p95 22.9 ms (PR #10) |
| R107 | Persistent "The work queue is full" and deletes still failing on shep.so after the fixes (18 September 2026) | Root cause: the running Shep dated from 13 September; the install replaced the binary but the launch handed over to the old process. `b07484e` makes a launch from a newer binary restart the owner and take over, or show a quit-and-reopen notice; the installer warns too. Sam must quit the old process once by hand |
| R106 | Extended e2e flows that move mail around, undo and keep moving quickly, because the UI drifts from the real moves with artefacts on rows (18 September 2026) | Fixture family `8a2231c` (eight `test_rapid_*` scenarios with store-truth and drawn-row checks; fixed a page left one row short after a move) and live family `7061394` against the approved `e2e@shep.so` mailbox (five `test_live_*` scenarios; Archive creation, `Deleted Items`, IDLE arrival, badge verified on the server). No remaining desync found; a release for Sam to confirm is next |
| R104 | Migrate untouched check-interval defaults to 5 seconds (15 September 2026) | `72da3c3`: database version 5 moves a saved 15 to 5 once; other values and a later deliberate 15 are kept |
| R105 | Avoid unsafe code unless absolutely necessary; record it in AGENTS.md (15 September 2026) | `c48bc08` adds the rule under "Architecture and data safety" |
| R103 | Use the safe and correct search query rather than unsafe FTS5 FFI, and raise the benchmark budget to match it (15 September 2026) | `e585cdb` reverted; search budgets set to 100 ms (one or two terms) and 150 ms (four terms) with page and body budgets unchanged; `performance-budgets.json` aligned on `fix/ci-inbox-page-benchmark` with a gate test against the benchmark. The runner benchmark passes on PR #10 (FTS p95 50.0 ms, transposed 42.9 ms, four terms 56.4 ms) |
| R102 | The taskbar badge shows a higher number than the current unread email count; it must equal the unread count at that moment (14 September 2026) | Fixed in `e4dcf5b` (lane `93c1656`): flag acknowledgements for mail not on the current page reconciled from the post-write state, leaving the old total published; the badge now publishes the projected total from the previously confirmed state. Sync-side flag ordering and dock render evidence remain open |
