...
| R41 | Add Forward and Print controls in preview | Forward delivered in 9062dcf: complete original/HTML/files, independent draft, both shortcut slots, background preparation/retry and five native flows; Print remains open |
...
| R58 | Additional useful features required | Existing sender actions, outgoing recovery, conversations, connection removal and calendar discovery and forwarding delivered; Print/selection/settings sync remain explicit open requests |
...
| R69 | HTML layout moves during rendering, visual artifacts and slow readiness; investigate more pre-caching/rendering | Open: reproduce and fix layout/frame stability, bounded adjacent-message preparation and stale-frame isolation; native/visual coverage required, timing deferred to the final idle-host checks |
...
| R71 | Refresh icon looks malformed in the latest screenshot | Open: locate affected native/browser control, inspect geometry/clipping/scaling and add visual regression coverage; location clarification pending |
