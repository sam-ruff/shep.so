...
| R22 | Encrypt local SQLite mail cache at rest | Open; keyed SQLite/journals, staged migration, encrypted selection scratch, bounded ancestry/order and verified exit-lifecycle foundation are implemented in codex/encrypted-cache; plain FILE/keyed MEMORY temp-policy correction pending shipping; guarded publication/recovery, conversation indexed scratch/DELETE-OFF correction awaits combined native validation; remaining sorter/portable paths and native startup validation remain |
