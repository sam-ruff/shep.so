...
| R32 | Multiple backup options simultaneously; good setup UX; Google Drive/S3/FTP/SFTP/local; compression and passcode encryption; restore password; rolling unreadable copies | Open beyond existing single Local/Drive configuration. `93d4289` fixes discovered CalDAV IDs/passwords in encrypted backup restore and rejects overlapping credential keys; full encrypt/decrypt/restore regression passes. |
