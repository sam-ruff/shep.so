...
| R69 | HTML layout moves during rendering, visual artifacts and slow readiness; investigate more pre-caching/rendering | In progress: actual viewport loading, stale-frame isolation, worker font reuse and bounded adjacent-frame preparation have Rust/native evidence; remaining controls/compact layout and final readiness work are tracked in TODO; timing deferred |
...
| R71 | Refresh icon looks malformed in the latest screenshot | In progress: shared native Mail/Calendar SVG arrowheads corrected and reviewed at normal/120% scale, light/dark and compact; exact browser-crop location remains unconfirmed |
