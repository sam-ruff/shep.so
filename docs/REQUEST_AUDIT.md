...
| R89 | Deleting a message should select the next message down and keep the list from snapping to the top | Adjacent optimistic selection, stable scroll, previous/empty fallback and bounded page refill implemented in the compact-mail lane; six controller and five native scenarios pass. Integration/push pending; see completion evidence |
