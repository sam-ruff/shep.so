...
- Move with the visible button or `M`, search a folder by name, accent-insensitive text, a typo or an abbreviation, then press Enter to use the highlighted top match. With "Search other accounts' folders when moving" on (it needs cross-account moves), typing also matches folders in other IMAP accounts: those rows carry a badge naming the account, rank a little below the current account's folders, and choosing one asks "Move to another account?" first, where Enter or Y moves and Escape or N returns to the folder list with the search intact. An empty search box still lists only the current account's folders, and an account chosen in the pick list still moves at once. Opt-in moves between IMAP accounts preserve the source until the destination confirms receipt; interrupted transfers are recorded to prevent blind duplicate uploads. Reply or Reply all, compose with To/Cc/Bcc, save drafts, archive, move to Trash, export original `.eml` files and save attachments through native file dialogs.
...
saves that listing; if the listing fails, the move fails before anything is sent.
