...
| Shared profile Drive transport | `shared/profile-core/src/drive/` | Optional native provider verifies the principal, owned bounded file pages and exact remote media against durable journal reservations. [Wire contract](PROFILE_DRIVE.md) and scripted HTTP tests exist; connect platform grants, durable discovery/catalog and real Settings/account application next. Live same-project access is unverified. |
