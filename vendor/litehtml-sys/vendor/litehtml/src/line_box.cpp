...
std::list< std::unique_ptr<litehtml::line_box_item> > litehtml::line_box::finish(bool last_box, const containing_block_context &containing_block_size)
{
    ...
    for (const auto& lbi : m_items)
    {
        ...
        // Inline fragments own their flow coordinates in lbi_start/lbi_end.
        // Their shared render element stores only the CSS relative offset.
        // Reset that offset before applying it: start/end, wrapped fragments,
        // table probes and subsequent renders must not accumulate it.
        if (lbi->get_type() != line_box_item::type_text_part)
        {
            lbi->get_el()->pos().move_to(0, 0);
        }
