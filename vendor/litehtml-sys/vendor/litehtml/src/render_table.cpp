...
namespace
{
#if SHEP_TABLE_LAYOUT_TESTS
    thread_local bool table_cache_enabled = true;
    thread_local uint64_t table_layouts = 0;
    thread_local uint64_t table_hits = 0;
#endif
    bool same_constraint(const litehtml::containing_block_context& a,
                         const litehtml::containing_block_context& b)
    {
        const auto equal = [](const auto& x, const auto& y) {
            return x.value == y.value && x.type == y.type;
        };
        return equal(a.width, b.width) && equal(a.render_width, b.render_width)
            && equal(a.min_width, b.min_width) && equal(a.max_width, b.max_width)
            && equal(a.height, b.height) && equal(a.min_height, b.min_height)
            && equal(a.max_height, b.max_height) && a.context_idx == b.context_idx
            && a.size_mode == b.size_mode;
    }
}

#if SHEP_TABLE_LAYOUT_TESTS
extern "C" void shep_table_layout_test_mode(bool enabled)
{
    table_cache_enabled = enabled;
    table_layouts = table_hits = 0;
}
extern "C" uint64_t shep_table_layout_test_count(bool hits)
{
    return hits ? table_hits : table_layouts;
}
#endif
...
litehtml::pixel_t litehtml::render_item_table::_render(pixel_t x, pixel_t y, const containing_block_context &containing_block_size, formatting_context* fmt_ctx, bool /*second_pass*/)
{
    ...
    auto epoch = src_el()->get_document()->layout_epoch();
#if SHEP_TABLE_LAYOUT_TESTS
    if (!table_cache_enabled) epoch = 0;
#endif
    if (epoch != 0 && m_layout_epoch == epoch &&
        same_constraint(m_layout_constraint, containing_block_size))
    {
#if SHEP_TABLE_LAYOUT_TESTS
        ++table_hits;
#endif
        // render_item::render recalculates outlines and clears our own box.
        // Descendant positions remain relative to this table and unchanged.
        m_pos.move_to(x + content_offset_left(), y + content_offset_top());
        m_pos.width = m_layout_width;
        m_pos.height = m_layout_height;
        return m_layout_return;
    }
    ...
    // Convert the caption origin to an actual height, including zero. A
    // caption exactly as tall as the border must still displace the cells.
    ...
    if (top_captions != 0)
    {
        ...
        for (int row = 0; row < m_grid->rows_count(); row++)
        {
            // Cells already have table-relative row coordinates. Moving the
            // row too double-counts the caption in selection/hit testing and
            // accumulates that offset across repeated table measurements.
            for (int col = 0; col < m_grid->cols_count(); col++)
            {
                table_cell* cell = m_grid->cell(col, row);
                if (cell->el)
    ...
    m_layout_constraint = containing_block_size;
    m_layout_epoch = epoch;
    m_layout_width = m_pos.width;
    m_layout_height = m_pos.height;
    m_layout_return = (self_size.width.type != containing_block_context::cbc_value_type_absolute
        ? std::min(table_width, max_table_width) : table_width) + content_offset_width();
    return m_layout_return;
}
