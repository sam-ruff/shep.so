...
namespace litehtml
{
	...
	{
	protected:
        // Only the most recent subtree layout is reusable, within one document
        // layout phase. A table's cells/captions own their formatting contexts.
        // Retain the complete input constraint, not the parent-adjusted pos().
        containing_block_context m_layout_constraint;
        uint64_t m_layout_epoch = 0;
        pixel_t m_layout_width = 0;
        pixel_t m_layout_height = 0;
        pixel_t m_layout_return = 0;
