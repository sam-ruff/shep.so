...
impl PixbufContainer {
    ...
    pub fn draw_selection_rects(&mut self, rects: &[litehtml::Position]) {
...
impl DocumentContainer for PixbufContainer {
    ...
    fn draw_image(
        ...
        let repeat_x = matches!(
            layer.repeat(),
            litehtml::BackgroundRepeat::Repeat | litehtml::BackgroundRepeat::RepeatX
        );
        let repeat_y = matches!(
            layer.repeat(),
            litehtml::BackgroundRepeat::Repeat | litehtml::BackgroundRepeat::RepeatY
        );
        let left = if repeat_x {
            clip.x
        } else {
            clip.x.max(origin.x)
        };
        ...
        let right = if repeat_x {
            clip.x + clip.width
        } else {
            (clip.x + clip.width).min(origin.x + origin.width)
        };
        let bottom = if repeat_y {
            clip.y + clip.height
        } else {
            (clip.y + clip.height).min(origin.y + origin.height)
        };
        ...
        if let Some(path) = build_rounded_rect_path(
            ...
            mask.fill_path(&path, FillRule::Winding, true, Transform::from_scale(s, s));
        } else {
            return;
        }
        if let Some(existing) = &self.cached_clip_mask {
            intersect_masks(&mut mask, existing);
        }
