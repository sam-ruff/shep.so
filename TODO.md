...
- [ ] **R69 — HTML rendering stability and readiness:** the reader moves as HTML renders, shows visual artifacts, and feels slow. Reproduce with realistic native fixtures; stabilize layout and frame replacement, investigate adjacent-message HTML pre-rendering/pre-caching on bounded background workers, and keep stale renders from flashing into the selected message. Cover rapid navigation, resize/scroll, images loading, light/dark and compact layouts with automated equivalents and reviewed WebP evidence. Defer performance measurements until the final idle-host work; do not infer speed from a loaded-host run.
  Current increment replaces provisional geometry with the real canvas viewport, reuses worker fonts/pixmaps, rejects obsolete frames and adds bounded background neighbor preparation; Rust regressions and targeted native/visual checks pass; all 98 native functional scenarios pass in one run; this increment is installed and shipped as cd8f732. Follow-up 6d83b72 discovers CSS/background references before display, moves horizontal panning inside the body, preserves Find results across resized frames, and restores compact body space above two-column attachments. All 100 native functional scenarios and 299 Rust/25 Python tests pass; release packaging passes. The final follow-up reproduces and fixes a 400-pixel late-image jump, preserves text anchors across image reflow with guarded native scrolling, and adds renderer Retry. New native and backend checks pass; the final full native rerun and shipping audit are underway before closing R69.
