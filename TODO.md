...
- [ ] **R15/R17/R21 — Final usability review:** finish remaining redundant-copy, spacing, icon sizing, alignment and compact/large-font checks, Singular message-count labels are corrected in the HTML-reader work; finish the remaining visual review. Preserve the approved dog logo, WebP assets, mouse usability, draggable panes and clean shadcn-style controls.
  The residual scale-control fragment below filtered settings is reproduced and fixed by the software-renderer clipping patch, with direct renderer and saved native pixel regressions. Final verification/shipping is in progress. Continue checking the previously reported clipped Shortcuts label and other compact/large-font states in the remaining visual audit.
