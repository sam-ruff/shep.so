...
- [ ] **R18/R44 — Fuzzy matching:** use a maintained matching library for inbox and Move search, with relevance ordering and typo tolerance. Regression: an email with body exactly “test” ranks ahead of weaker matches. Keep stable paging and stale-query protection. Implementation now uses RapidFuzz with explicit Best match ordering; exact-body, paging/reopen, scope and Unicode tests pass. Native verification and shipping are in progress; a discovered Move-during-body-load race now uses selected metadata and rejects stale reader bodies.
...
- [ ] **R15/R17/R21 — Final usability review:** finish remaining redundant-copy, spacing, icon sizing, alignment and compact/large-font checks, including singular message-count labels. Preserve the approved dog logo, WebP assets, mouse usability, draggable panes and clean shadcn-style controls.
