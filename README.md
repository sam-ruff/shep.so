...
- Drafts autosave after editing pauses. Closing waits for a successful save, and failures keep the composer open. Older saves cannot overwrite newer edits or restore a sent draft. SMTP rejection preserves the draft; if delivery cannot be confirmed, check Sent before retrying. Sent messages currently have a local Sent copy.
...
- SMTP sends retain a local Sent copy; server-side Sent APPEND is not implemented yet. Editing recurring CalDAV series is not implemented. Global Google sign-out and a disconnect option that retains an offline archive remain in development.
